...
static int report(int total, int pass)
{
  ...
  cout << "Dealer: Passing " << pass << " of " << total << " tests" << endl;
  if(pass != total){
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
...
int main()
{
  ...
  int passing = 0;
  int total = 0;
  //Test the constructor works for a dealer
  Dealer *p = new Dealer();
  ...
  //Now Make sure that dealer methods work
  //Deal some cards and see what we have
  Deck *d = new Deck();
  p->hitCard(c1);
  p->hitCard(d->dealCard());

  //See if outputs are right
  total++;
  cout << p->showInitialHand() << endl;
  if(p->showInitialHand().compare("Dealer has Ace of Spades")){
    cerr << "Dealer should have an ace of spades showing" << endl;
    delete p;
  ...
  //Now test the play methods
  total++;
  p->playHand(d);
  if(p->getScore() < 17){
    cerr << "Playing a hand should result in a 17 or greater for the dealer" << endl;
    delete p;
    exit(report(total, passing));
  }
  passing++;
  ...
  //See if we can print out the right stuff
  cout << *p << endl;
